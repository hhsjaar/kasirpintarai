...
export default function Dashboard({ refreshTrigger, onRefreshCompleted }: DashboardProps) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [resProd, resTxn, resNotif, resKas] = await Promise.all([
        fetch('/api/products'),
  ...
  const handleSettleKasbon = async (kasbonId: string) => {
    if (!confirm('Apakah Anda yakin ingin melunasi kasbon ini?')) return;
    try {
      const res = await fetch('/api/kasbon', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ kasbonId })
      });
      if (res.ok) {
        fetchData();
      } else {
  ...
  const totalUnpaidKasbon = kasbons
    .filter(k => k.status === 'UNPAID')
    .reduce((acc, k) => acc + k.amount, 0);

  const unpaidKasbonCount = kasbons.filter(k => k.status === 'UNPAID').length;
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="flex border-b border-slate-200 gap-6 text-sm font-semibold">
        <button
          ...
          className={`pb-3 relative transition-all cursor-pointer ${
            activeTab === 'analytics' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          Ringkasan & Analisis
        </button>
        <button
          ...
          className={`pb-3 relative transition-all cursor-pointer ${
            activeTab === 'inventory' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          Gudang & Inventori
        </button>
        <button
          ...
          className={`pb-3 relative transition-all cursor-pointer ${
            activeTab === 'kasbon' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          Manajemen Kasbon
          {unpaidKasbonCount > 0 && (
            <span className="ml-1.5 px-1.5 py-0.5 text-3xs font-black bg-rose-105 text-rose-600 rounded-full">
              {unpaidKasbonCount}
            </span>
          )}
        </button>
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-8 animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div className="bg-white border border-slate-100 border-l-4 border-l-rose-500 p-5 rounded-2xl flex flex-col justify-between h-32 hover:scale-[1.02] transition-transform duration-300 shadow-sm shadow-slate-100/50">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Piutang Kasbon</span>
                <div className={`p-2 rounded-lg ${totalUnpaidKasbon > 0 ? 'bg-rose-50 text-rose-600 animate-pulse' : 'bg-slate-100 text-slate-400'}`}>
                  <Activity className="w-4 h-4" />
                </div>
              </div>
              <div>
                <h3 className={`text-xl font-extrabold ${totalUnpaidKasbon > 0 ? 'text-rose-600' : 'text-slate-800'}`}>
                  Rp {totalUnpaidKasbon.toLocaleString('id-ID')}
                </h3>
                <p className="text-slate-405 text-2xs mt-1">
                  {unpaidKasbonCount} Kasbon Belum Lunas
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Tab 3: Manajemen Kasbon */}
      {activeTab === 'kasbon' && (
        <div className="space-y-6 animate-fade-in">
          {/* Kasbon Metrics Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="bg-white border border-slate-100 border-l-4 border-l-rose-500 p-5 rounded-2xl flex flex-col justify-between h-32 hover:scale-[1.02] transition-transform duration-300 shadow-sm shadow-slate-100/50">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Piutang Toko</span>
                <div className="p-2 bg-rose-50 rounded-lg text-rose-600">
                  <TrendingUp className="w-4 h-4" />
                </div>
              </div>
              <div>
                <h3 className="text-xl font-extrabold text-slate-800">Rp {totalUnpaidKasbon.toLocaleString('id-ID')}</h3>
                <p className="text-slate-400 text-2xs mt-1">
                  Total dana yang belum dilunasi pembeli
                </p>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-100 border-l-4 border-l-amber-500 p-5 rounded-2xl flex flex-col justify-between h-32 hover:scale-[1.02] transition-transform duration-300 shadow-sm shadow-slate-100/50">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Jumlah Kasbon Aktif</span>
                <div className="p-2 bg-amber-50 rounded-lg text-amber-600">
                  <ClipboardList className="w-4 h-4" />
                </div>
              </div>
              <div>
                <h3 className="text-xl font-extrabold text-slate-800">{unpaidKasbonCount} Transaksi</h3>
                <p className="text-slate-405 text-2xs mt-1">
                  Jumlah pembeli yang berutang aktif
                </p>
              </div>
            </div>
          </div>
                      ...
                      <tr key={k.id} className="hover:bg-slate-50/50 transition text-slate-650">
                        ...
                        <td className="py-3.5 px-4">
                          {k.status === 'PAID' ? (
                            ...
                            <span className="inline-block px-2.5 py-0.5 rounded-full text-3xs font-black bg-rose-50 text-rose-600 uppercase">
                              Belum Lunas
                            </span>
                          )}
                        </td>
                        <td className="py-3.5 px-4 text-center">
                          {k.status === 'UNPAID' ? (
                            <button
                              onClick={() => handleSettleKasbon(k.id)}
